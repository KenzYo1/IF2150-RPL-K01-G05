...
||| *Sistem menampilkan halaman laporan yang berisi formulir laporan* |
| 2 | *Pelapor mengisi formulir laporan dengan lengkap* ||
||| *Sistem menerima data dari kolom-kolom yang diisi pelapor pada _front-end_ atau antarmuka dan secara bersamaan melakukan validasi teknis (misal, jumlah karakter deskripsi laporan, jenis file foto)* |
| 3 | *Pelapor mengonfirmasi pengiriman laporan* ||
||| *Sistem menerima data formulir laporan dan mengirimnya ke database sebagai daftar laporan untuk ditanggapi Petugas atau Admin* |
...
||| *Sistem menampilkan halaman formulir laporan* |
| 2 | *Pelapor mengisi formulir laporan, tetapi salah satu aspek teknis laporan (misal, jumlah karakter tidak melebihi batas, jenis file foto sesuai, dan kolom yang wajib diisi terisi semua) tidak terpenuhi* |
||| *Sistem menampilkan pesan _error_ yang sesuai dengan aspek teknis yang tidak terpenuhi* |
| 3 | *Pelapor menyesuaikan laporannya* ||
||| *Sistem kembali ke langkah 2 skenario normal* |
...
||| *Sistem menampilkan seluruh laporan beserta status laporan tersebut* |
| 2 | *Petugas mengubah status salah satu laporan (misal, penerimaan, penolakan, dan _progress_ berupa tanggapan)* ||
||| *Sistem memperbarui data status laporan pada _front-end_ atau antarmuka untuk dikirim nantinya* |
| 3 | *Petugas menekan tombol untuk mengirim perubahan status laporan* ||
||| *Sistem mengirimkan data perubahan status laporan ke basis data* |
||| *Sistem mengirimkan notifikasi perubahan laporan ke pelapor yang berkaitan* | 
||| *Sistem memberi poin jika laporannya diterima, sebaliknya laporan dihapus jika ditolak* |
<br>
...
||| *Sistem menampilkan daftar laporan beserta status dari laporan tersebut* |
| 2 | *Admin mengubah salah satu status laporan karena alasan administratif, seperti laporan yang tidak senonoh, spam, laporan palsu, dan sebagainya* ||
||| *Sistem memperbarui data status laporan pada _front-end_ atau antarmuka untuk dikirim nantinya* |
| 3 | *Admin menekan tombol untuk mengirim perubahan status laporan* ||
||| *Sistem mengirimkan data perubahan status laporan ke basis data* |
||| *Sistem mengirimkan notifikasi perubahan laporan ke pelapor yang berkaitan* | 
||| *Sistem memberi poin jika laporannya diterima, sebaliknya laporan dihapus jika ditolak* |
